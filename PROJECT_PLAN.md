...
- [x] **番号打分匹配与分流过滤算法**：重构单一正则，引入带横杠/无横杠双通道自适应过滤模式。不仅能彻底剔除文件名中混杂的 `aavv121.com`、`hhd800.com` 等垃圾域名污染，而且完美兼容 `T38-041` 这种以“单字母+数字”为前缀的特殊番号，同时对多余前导零（如 `00435` 归集为 `435`）进行了高准度规范化。
